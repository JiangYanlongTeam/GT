...
<%
	...
	if(!"".equals(detailrequest)) {
		// 子流程字段转换
		String _detailrequestSQL = "";
		if("62".equals(workflowid) || "63".equals(workflowid)){
			String BMBLWORKFLOWID = new weaver.general.BaseBean().getPropValue("GWSWBL", "BMBLWORKFLOWID");
			_detailrequestSQL = "select a.requestid,a.requestname,(select departmentname from hrmdepartment where id = (select departmentid from hrmresource where id = a.creater)) departmentname from workflow_requestbase a where a.requestid in (select requestid from workflow_requestbase where mainrequestid = (select mainrequestid from workflow_requestbase where requestid = '"+reqid+"') and workflowid = "+BMBLWORKFLOWID+")";
		} else if("61".equals(workflowid) || "59".equals(workflowid) || "181".equals(workflowid) || "201".equals(workflowid)){
			_detailrequestSQL = "select a.requestid,a.requestname,(select departmentname from hrmdepartment where id = (select departmentid from hrmresource where id = a.creater)) departmentname from workflow_requestbase a where a.requestid in (select requestid from workflow_requestbase where mainrequestid = '"+reqid+"')";
		} else {
		...
		%>
		<script>
			//jQuery("input [name=field<%=detailrequest%>]").append("<%=request_a%>");
	...
	if(!"".equals(detailhistory)) {
		...
		String _detailrequestSQL1 = "select distinct requestid,requestname from ofs_todo_data where requestname = (select requestname from workflow_requestbase where requestid = '"+reqid+"')";
		...
		if(userid == Integer.parseInt(hrmid)) {
			...
			<script>
				...
				if(jQuery("#field<%=fieldid%>").attr("type") != "text") {
					jQuery("#field<%=fieldid%>").parent().after(jQuery("<img src='/weaver/weaver.file.ImgFileDownload?userid="+_fval+"' width='50px'>"));
				} else {
					jQuery("#field<%=fieldid%>").parent().parent().parent().parent().parent().after(jQuery("<img src='/weaver/weaver.file.ImgFileDownload?userid="+_fval+"' width='50px'>"));
				}
			}
				jQuery("#field<%=jld_qz%>_span").html("<input type=\"button\" onclick=\"sign('<%=hrmid%>','<%=fieldid%>','<%=date%>','<%=rqid%>')\"class=\"e8_btn_top_first\" _disabled=\"true\" value=\"签名\" title=\"签名\" style=\"max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;width:80px;background-color:red;color:white;\">");
			</script>
			<%
		} else {
			%>
	...
	while(rs.next()){
		...
		<script>
			...
			jQuery("#field<%=jld_qz%>_span").html("<input type=\"button\" onclick=\"sign('<%=userid%>','<%=fieldid%>','<%=date%>','<%=rqid%>')\"class=\"e8_btn_top_first\" _disabled=\"true\" value=\"签名\" title=\"签名\" style=\"max-width: 100px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;width:80px;background-color:red;color:white;\">");
		</script>
		<%
	}
...
<script language="javascript">
	...
	function sign(obj,obj2,obj3,obj4){
		if(jQuery("#field"+obj2).attr("type") != "text") {
			...
			jQuery("#field"+obj2).parent().after(jQuery("<img src='/weaver/weaver.file.ImgFileDownload?userid="+obj+"' width='50px'>"));
		} else {
			jQuery("#field"+obj2).parent().parent().parent().parent().parent().parent().find('img').remove();
			jQuery("#field"+obj2).parent().parent().parent().parent().parent().after(jQuery("<img src='/weaver/weaver.file.ImgFileDownload?userid="+obj+"' width='50px'>"));
		}
	...
	dosubmit = function (btnobj){
		...
		if("<%=workflowid%>" == "61" && ("<%=nodeid%>" == "282")) {
			...
			jQuery.get('/interface/jiangyl/tip/updatejld.jsp',{requestid:requestid,jld1:jld1value,jld2:jld2value,jld3:jld3value,jld4:jld4value,jld5:jld5value,jld6:jld6value,jld7:jld7value,jld8:jld8value,jld9:jld9value,jld10:jld10value,jldrq1:jldrq1value,jldrq2:jldrq2value,jldrq3:jldrq3value,jldrq4:jldrq4value,jldrq5:jldrq5value,jldrq6:jldrq6value,jldrq7:jldrq7value,jldrq8:jldrq8value,jldrq9:jldrq9value,jldrq10:jldrq10value,jzyj1:jzyj1value,jzyj2:jzyj2value,jzyj3:jzyj3value,jzyj4:jzyj4value,jzyj5:jzyj5value,jzyj6:jzyj6value,jzyj7:jzyj7value,jzyj8:jzyj8value,jzyj9:jzyj9value,jzxr:jzxrvalue,jzxbm1:jzxbm1value,jzxbm2:jzxbm2value,jzxbm3:jzxbm3value,jzxbm4:jzxbm4value,jzxbm5:jzxbm5value,jzxbm6:jzxbm6value,jzxbm7:jzxbm7value,jzxbm8:jzxbm8value,jzxbm9:jzxbm9value,jzxr2:jzxr2value,jzxr3:jzxr3value,jzxr4:jzxr4value,jzxr5:jzxr5value,jzxr6:jzxr6value,jzxr7:jzxr7value,jzxr8:jzxr8value,jzxr9:jzxr9value,jzxcs:jzxcsvalue,jzxsydw:jzxsydwvalue,jzxfj:jzxfjvalue,jzxbm10:jzxbm10value},function(data){
				flag = true;
			})
		}
		if("<%=workflowid%>" == "61" && ("<%=nodeid%>" == "282")) {
			 ...
			 var ssbm2= "";
			 var _qzyj = "";
			 var _reQID = jQuery("input[name=requestid]").val();
		...
		if("<%=workflowid%>" == "201" && ("<%=nodeid%>" == "424")) {
			...
			var requestid = jQuery("input[name=requestid]").val();
			jQuery.get('/interface/jiangyl/tip/update_hytzjld.jsp',{requestid:requestid,jld1:jld1value,jld2:jld2value,jld3:jld3value,jld4:jld4value,jld5:jld5value,jld6:jld6value,jld7:jld7value,jld8:jld8value,jld9:jld9value,jld10:jld10value,jldrq1:jldrq1value,jldrq2:jldrq2value,jldrq3:jldrq3value,jldrq4:jldrq4value,jldrq5:jldrq5value,jldrq6:jldrq6value,jldrq7:jldrq7value,jldrq8:jldrq8value,jldrq9:jldrq9value,jldrq10:jldrq10value,jzyj1:jzyj1value,jzyj2:jzyj2value,jzyj3:jzyj3value,jzyj4:jzyj4value,jzyj5:jzyj5value,jzyj6:jzyj6value,jzyj7:jzyj7value,jzyj8:jzyj8value,jzyj9:jzyj9value,jzxr:jzxrvalue,jzxbm1:jzxbm1value,jzxbm2:jzxbm2value,jzxbm3:jzxbm3value,jzxbm4:jzxbm4value,jzxbm5:jzxbm5value,jzxbm6:jzxbm6value,jzxbm7:jzxbm7value,jzxbm8:jzxbm8value,jzxbm9:jzxbm9value,jzxr2:jzxr2value,jzxr3:jzxr3value,jzxr4:jzxr4value,jzxr5:jzxr5value,jzxr6:jzxr6value,jzxr7:jzxr7value,jzxr8:jzxr8value,jzxr9:jzxr9value,jzxcs:jzxcsvalue,jzxsydw:jzxsydwvalue,jzxfj:jzxfjvalue,jzxbm10:jzxbm10value},function(data){
				flag = true;
			})
		}
			...
			if(flag) {
				if ("<%=workflowid%>" == "61"){
					if (confirm("是否确认提交机要室整理？")){
						return  __systemHandleFunction(btnobj);
					} else {
						return false;
					}
				}
	...
	function dowrite(gdtxr,ssbm, ssbm1, ssbm2, _reQID, _reQNAME, _qzyj){
		jQuery.get('/interface/jiangyl/tip/ldpsyj.jsp',{gdtxr:gdtxr,ssbm:ssbm,ssbm1:ssbm1,ssbm2:ssbm2,requestid:_reQID,reqName:_reQNAME,qzyj:_qzyj},function(data){
