			...
			<td colspan="2">
				...
				<tbody>
					...
					if("".equals(_requestid) || "undefined".equals(_requestid)){
						...
						String sql = "select requestid,swrq bt from " + tableName + " where  requestid in ("+"select requestid from workflow_requestbase where requestid != '"+mainRequestId+"' and workflowid= '61' and requestname = (select requestname from workflow_requestbase where requestid = '"+mainRequestId+"'))";
						new weaver.general.BaseBean().writeLog("sql："+sql);
						RecordSet.execute(sql);
						while(RecordSet.next()) {
							...
							sb2.append("<a style=\"cursor:pointer;font-family:微软雅黑 !important;font-weight:bold;\" onclick=\"openWf('"+reqid1+"')\" title=\""+bt1+"\">"+bt1+"</a>&nbsp;");
							sb2.append("</span>");
							sb2.append("</div>");
							sb2.append("</div>");
							sb2.append("</td>");
						...
						String atsql ="select distinct flowid,requestid,workflowid,createdate from ofs_todo_data a where a.workflowname = '公文办理收文_市局' and requestname = (select requestname from workflow_requestbase where requestid = "+mainRequestId+")";
						RecordSet.execute(atsql);
							...
							sb2.append("<div style=\"float:left;margin-left:5px;\">");
							sb2.append("<span style=\"display:inline-block;width:100%;height:30px;padding-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;vertical-align: middle;\">");
							sb2.append("<a style=\"cursor:pointer;font-family:微软雅黑 !important;font-weight:bold !important;color:black !important;\" onclick=\"openWf2('"+flowid+"','"+workflowid+"')\" title=\""+bt+"\">"+bt+"</a>&nbsp;");
							sb2.append("</span>");
							sb2.append("</div>");
							sb2.append("</div>");
							sb2.append("</td>");
