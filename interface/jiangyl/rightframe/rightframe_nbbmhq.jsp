			...
			<td colspan="4">
				...
				<tbody>
					...
					if("".equals(_requestid) || "undefined".equals(_requestid)){
						...
						String sql = "select glswnew,(select requestname from workflow_requestbase where requestid = glswnew) reqname from " + _tablename + " where requestid = "+_requestid+"";
						new weaver.general.BaseBean().writeLog("sql:"+sql);
						RecordSet.execute(sql);
						RecordSet.next();
						String fj = Util.null2String(RecordSet.getString("glswnew"));
						String reqname = Util.null2String(RecordSet.getString("reqname"));
						if("".equals(fj)) {
							
						} else {
							if(Integer.parseInt(fj) < 0) {
								RecordSet.execute("select distinct requestname from ofs_sysinfo s ,ofs_todo_data d where d.sysid=s.sysid and d.requestid="+fj+" and s.sysid=-1 and  islasttimes=1 ");
								RecordSet.next();
								reqname = Util.null2String(RecordSet.getString(1));
				...
				<table class="LayoutTable" style="width:100%;height:30px;">
					...
					<tbody>
						<tr class="groupHeadHide" style="background-color: #F2F2F2">
							<td class="interval" style="border-left-style: solid !important;border-left-color: rgb(0, 112, 192) !important;">
								<span class="groupbg" style=""></span>
								<span class="e8_grouptitle" style="height: 30px; vertical-align: middle; white-space: nowrap; text-overflow: ellipsis; word-break: keep-all; overflow: hidden;">公文拟报</span>
							</td>
							<td class="interval" colspan="2" style="text-align:right;">
								<span class="toolbar">	
								</span>															
		...
		<tr class="items intervalTR" style="display:">
			<td colspan="4">
				...
				<tbody>
					...
					<%
					String mainsql = "select requestid,requestname from workflow_requestbase where requestid = (select mainrequestid from workflow_requestbase where requestid = '"+_requestid+"')";
					RecordSet.execute(mainsql);
					RecordSet.next();
					String mainRequestid = Util.null2String(RecordSet.getString("requestid"));
					String mainRequestname = Util.null2String(RecordSet.getString("requestname"));
					StringBuffer sb3 = new StringBuffer();
					
					if("".equals(mainRequestid)) {
						
					} else {
						sb3.append("<tr style=\"border-bottom:1px solid #e6e6e6;height: 40px;\">");
						sb3.append("<td class=\"fieldvalueClass\" valign=\"middle\" colspan=\"2\" style=\"word-break:normal;word-wrap:normal;\">");
						sb3.append("<div style=\"float: left; height: 40px; line-height: 40px; width: 100%; background-color: rgb(255, 255, 255);\" class=\"fieldClassChange\">");
						sb3.append("<div style=\"float:left;margin-left:5px;\">");
						sb3.append("<span style=\"display:inline-block;width:100%;height:30px;padding-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;vertical-align: middle;\">");
						sb3.append("<a style=\"cursor:pointer;font-family:微软雅黑 !important;font-weight:bold;\" onclick=\"openWf('"+mainRequestid+"')\" title=\""+mainRequestname+"\">"+mainRequestname+"</a>&nbsp;");
						sb3.append("</span>");
						sb3.append("</div>");
						sb3.append("</div>");
						sb3.append("</td>");
		...
		<tr class="intervalTR" _samepair="showgroup" style="display:">
			<td colspan="4">
				<table cellspacing="0" class="ListStyle" style="table-layout: fixed;">
					...
					<tbody>
					<%
						weaver.interfaces.action.GWNB GWNB = new weaver.interfaces.action.GWNB();
						...
						java.util.List<java.util.Map<String, String>> listMap = GWNB.getHQData(user,utMap,request,response);
