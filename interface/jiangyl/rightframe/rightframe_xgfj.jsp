		...
		<tr class="items intervalTR" style="display:">
			<td colspan="2">
				...
				<tbody>
					...
					if("".equals(_requestid) || "undefined".equals(_requestid) || "-1".equals(_requestid)){
			...
			<td colspan="2">
				...
				<tbody>
					...
					if("".equals(_requestid) || "undefined".equals(_requestid) || "-1".equals(_requestid)){
						...
						String sql ="select requestid,swrq from "+_tablename+" where requestid in ("+"select requestid from workflow_requestbase where requestid != '"+_requestid+"' and workflowid= '61' and requestname = (select requestname from workflow_requestbase where requestid = '"+_requestid+"'))";
						new weaver.general.BaseBean().writeLog("sql:"+sql);
						RecordSet.execute(sql);
						while(RecordSet.next()) {
							String reqid = Util.null2String(RecordSet.getString("requestid"));
							String bt = Util.null2String(RecordSet.getString("swrq"));
							sb1.append("<tr style=\"border-bottom:1px solid #e6e6e6;height: 40px;\">");
							sb1.append("<td class=\"fieldvalueClass\" valign=\"middle\" colspan=\"2\" style=\"word-break:normal;word-wrap:normal;\">");
							sb1.append("<div style=\"float: left; height: 40px; line-height: 40px; width: 100%; background-color: rgb(255, 255, 255);\" class=\"fieldClassChange\">");
							sb1.append("<div style=\"float:left;margin-left:5px;\">");
							sb1.append("<span style=\"display:inline-block;width:100%;height:30px;padding-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;vertical-align: middle;\">");
							sb1.append("<a style=\"cursor:pointer;font-family:微软雅黑 !important;font-weight:bold !important;color:black !important;\" onclick=\"openWf('"+reqid+"')\" title=\""+bt+"\">"+bt+"</a>&nbsp;");
							sb1.append("</span>");
							sb1.append("</div>");
							sb1.append("</div>");
							sb1.append("</td>");
						...
						String atsql ="select distinct flowid,requestid,workflowid,createdate from ofs_todo_data a where a.workflowname = '公文办理收文_市局' and requestname = (select requestname from workflow_requestbase where requestid = "+_requestid+")";
						RecordSet.execute(atsql);
						while(RecordSet.next()) {
							String reqid = Util.null2String(RecordSet.getString("requestid"));
							String flowid = Util.null2String(RecordSet.getString("flowid"));
							String bt = Util.null2String(RecordSet.getString("createdate"));
							...
							sb1.append("<div style=\"float:left;margin-left:5px;\">");
							sb1.append("<span style=\"display:inline-block;width:100%;height:30px;padding-bottom:10px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;vertical-align: middle;\">");
							sb1.append("<a style=\"cursor:pointer;font-family:微软雅黑 !important;font-weight:bold !important;color:black !important;\" onclick=\"openWf2('"+flowid+"','"+workflowid+"')\" title=\""+bt+"\">"+bt+"</a>&nbsp;");
							sb1.append("</span>");
							sb1.append("</div>");
							sb1.append("</div>");
							sb1.append("</td>");
		...
		<tr class="intervalTR" _samepair="showgroup" style="display:">
			<td colspan="2">
				<table cellspacing="0" class="ListStyle" style="table-layout: fixed;">
					...
					<tbody>
					<%
						weaver.interfaces.action.GWSWBL GWSWBL = new weaver.interfaces.action.GWSWBL();
						if("".equals(_requestid) || "undefined".equals(_requestid) || "-1".equals(_requestid)){
							...
							java.util.List<java.util.Map<String, String>> listMap = GWSWBL.getData(user,utMap,request,response);
...
function openWf2(id,wfid) {
	window.open("http://192.168.1.2/NetOfficeWeb/FrameWork/Views/Office/WorkView.html?iid="+id);
}
</script>
 </body>
</html>