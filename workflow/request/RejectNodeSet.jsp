...
%>
<%!
public String getCurrentOperator(String nodeid, String requestid){
	weaver.conn.RecordSet rs = new weaver.conn.RecordSet();
	String sql = "select distinct a.nodeid,b.nodename,a.USERID as hrmids,a.NEEDWFBACK from workflow_currentoperator a,workflow_nodebase b where a.nodeid=b.id and a.requestid='"+requestid+"' and a.nodeid = '"+nodeid+"' order by a.nodeid,b.nodename";
	rs.execute(sql);
	int count = rs.getCounts();
	if(count > 1) {
		String sql1 = "select distinct a.nodeid,b.nodename,listagg(a.USERID,',') WITHIN GROUP (order by a.USERID) hrmids from workflow_currentoperator a,workflow_nodebase b where a.nodeid=b.id and a.requestid='"+requestid+"' and a.nodeid = '"+nodeid+"' and a.NEEDWFBACK = '1' group by b.nodename,a.nodeid order by a.nodeid,b.nodename";
		rs.execute(sql1);
	}
	rs.next();
	String userid = new weaver.general.Util().null2String(rs.getString("hrmids"));
	if(userid.contains(",")) {
		java.lang.StringBuffer sb = new java.lang.StringBuffer(",");
		java.util.Map<String,String> _map = new java.util.HashMap<String,String>();
		String[] strs = userid.split(",");
		for(int i =0;i < strs.length; i++) {
			_map.put(strs[i],strs[i]);
		}
		for(java.util.Map.Entry<String,String> entry : _map.entrySet()) {
	...
	return resource.getLastnames(userid);
}
%>
<html>
<head>
	     ...
	     <wea:item attributes="{'isTableList':'true'}">
	        <table class="ListStyle" id="oTable" cellspacing=0>
				...
				%>
						    ...
						    <td>
						        <%=getCurrentOperator(Util.null2String(rejectnodeids.get(i)),""+requestid)%>
						    </td>
						</tr>
				  <%
