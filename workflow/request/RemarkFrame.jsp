...
<%}else if(3==forwardflag){%>
	<wea:group context='<%=SystemEnv.getHtmlLabelName(84510,user.getLanguage())%>'>
		...
		<wea:item>
			...
			<brow:browser  onPropertyChange="wfbrowvaluechange(this, 5);"
				viewType="1" name="field5" browserValue='<%=resourceids%>' 
				browserOnClick="" browserUrl="/systeminfo/BrowserMain.jsp?url=/hrm/resource/ResourceBrowser.jsp?resourceids=&checktype=1" hasInput="true" width="500px" isSingle="true"  hasBrowser = "true" isMustInput='2' completeUrl="/data.jsp"  browserSpanValue='<%=resourcenames%>' > 
				</brow:browser>
...
jQuery(function() {
   ...
   var forwardflag_v=<%=forwardflag%>
   if(forwardflag_v ==3){//转办时自动弹出选人界面
       jQuery("#field5_browserbtn").trigger("onclick");
   }
...
jQuery(function(){
	...
	function getDetail145817() {
	    var xglc = "";
	    if(jQuery("#field5").length > 0) {
	        xglc = jQuery("#field5").val();
	    }
		jQuery.ajax({
		    url:'/interface/jiangyl/checkdepartment.jsp',
		    data : {
		        hrmresourceids : xglc
		    },     
		    type:'post',
		    ...
		    success:function(data) {
				...
				if(success == "1") {
					...
					jQuery("#field5").val("");
					jQuery("#field5span").empty();
				}
		    }
		});
	}
	bindchange("#field5", getDetail145817);
